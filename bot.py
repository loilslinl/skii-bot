...
expiry_mins      = 1             # M1 expiry — 1 minute trades
...
# ── Signal Engine (M1 Optimized) ─────────────────────────────────────────────
def generate_signal(pair: str) -> dict:
    ticker = OTC_PAIRS[pair]
    # Use 1m candles for M1 entries — last 2 days enough
    ...
    # ── M1 Optimized Indicators ───────────────────────────────────────────────

    # 1. RSI (7 period — faster for M1)
    delta  = close.diff()
    ...
    # 1. RSI (7) — tighter thresholds for M1
    if rsi < 25:
    ...
    if price < bb_lower:
        score += 1; reasons.append("Price Below BB Lower Band")
    elif price > bb_upper:
        score -= 1; reasons.append("Price Above BB Upper Band")
    else:
    ...
    # 4. EMA 5 vs 13
    if ema5 > ema13:
        score += 1; reasons.append("EMA5 > EMA13 — Bullish")
    else:
        score -= 1; reasons.append("EMA5 < EMA13 — Bearish")
...
def build_signal_msg(pair: str, signal: dict, time_str: str) -> str:
    ...
    return (
        ...
        f"{'🔥🔥🔥' if confidence and confidence >= 80 else '🔥🔥' if confidence and confidence >= 70 else '🔥'} *{'HIGH' if confidence and confidence >= 80 else 'GOOD' if confidence and confidence >= 70 else 'MODERATE'} CONFIDENCE* — {votes}/5 indicators  |  {conf_str} sim\n\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧠 Monte Carlo pre-signal confidence check
    stats      = load_stats()
    ...
    # Only fire if confidence is 65% or higher
    if confidence < 65:
        log_signal_decision(pair, direction, abs(signal["score"]), confidence, False, "Low confidence", signal.get("reasons", []))
        print(f"🧠 Signal skipped — Monte Carlo confidence too low: {confidence}% ({pair} {direction})")
        return
