...
def get_mtf_trend(ticker: str) -> dict:
    ...
    for tf, (period, interval, fast, slow) in configs.items():
        try:
            df    = get_cached_df(ticker, period, interval)
            if df.empty or len(df) < slow + 5:
                results[tf] = "NEUTRAL"; continue
            ...
            results[tf] = "UP" if ef > es * 1.0001 else "DOWN" if ef < es * 0.9999 else "NEUTRAL"
        except Exception:
            results[tf] = "NEUTRAL"
    ...
    results["overall"]  = "UP" if up_count >= 2 else "DOWN" if down_count >= 2 else "NEUTRAL"
    results["strength"] = max(up_count, down_count)
    return results
...
def get_mtf_trend(ticker: str) -> str:
    try:
        df    = get_cached_df(ticker, "1d", "5m")
