...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    """Scans all pairs with all 8 indicators. Fires best signal every 5 min."""
    if bot_paused:
        return
    ...
    best_signal = None  # (pair, direction, indicator, reason, entry_price)
    ...
    for pair in active_pairs:
        ...
        for indicator_name, check_fn in INDICATOR_CHECKS.items():
            ...
            # Pick first valid signal found (already filtered by cooldown)
            if best_signal is None:
                best_signal = (pair, direction, indicator_name, reason, data["price"])
    ...
    if not best_signal:
        print("🔍 Scan complete — no new signals this cycle.")
        return

    pair, direction, indicator_name, reason, entry_price = best_signal

    # Set cooldowns
    set_indicator_cooldown(indicator_name, pair)
    last_signal_time = datetime.now(timezone.utc)

    # 🎮 Prediction game in free group — ask before revealing signal
    try:
        await context.bot.send_poll(
            ...
            options=["📈 CALL", "📉 PUT"],
            is_anonymous=False,
            open_period=120,  # poll open for 2 minutes
        )
    except Exception as e:
        print(f"⚠️ Prediction poll failed: {e}")

    # Wait 2 minutes then reveal to free group who was right
...
async def reveal_prediction_job(context: ContextTypes.DEFAULT_TYPE):
    """Reveals which direction was correct to free group after 2 min."""
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
                f"🔒 *Paid members already placed this trade!*\n"
                f"Get signals before the reveal 👇\n"
                f"👉 *whop.com/skiiprosignals*\n\n"
                f"_🎿 Skii Pro Signals — Premium OTC Alerts_"
            )
        )
    except Exception as e:
        print(f"⚠️ Reveal job failed: {e}")
...
async def result_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=FREE_GROUP_ID, text=free_text, parse_mode="Markdown")
    except Exception:
        pass
...
async def stats_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=FREE_GROUP_ID, text=text, parse_mode="Markdown")
