...
return (
    ...
    f"   • CCI         : `{signal.get('cci', 'N/A')}`\n"
    f"   • Williams %R : `{signal.get('williams_r', 'N/A')}`\n"
    f"   • ADX         : `{signal.get('adx', 'N/A')}`\n"
    f"   • Stoch %K    : `{signal.get('stoch_k', 'N/A')}`\n\n"
    f"🔥🔥🔥 *HIGH CONFIDENCE* — {votes}/8\n\n"
    f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
...
for pair in active_pairs:
    ...
    # Only consider signals with 6+ indicators agreeing
    if abs(signal["score"]) < 6:
        continue
...
if not best_signal:
    print("🔍 Scan complete — no signal met 6/8 threshold.")
    return
