...
def log_signal_decision(pair: str, direction: str, score: int, confidence: float, fired: bool, reason: str = ""):
...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for pair in active_pairs:
        ...
        if abs(signal["score"]) < 3:
            log_signal_decision(pair, signal["direction"], abs(signal["score"]), 0, False, "Score too low")
            continue
    ...
    if confidence < 65:
        log_signal_decision(pair, direction, abs(signal["score"]), confidence, False, "Low confidence")
        print(f"🧠 Signal skipped — Monte Carlo confidence too low: {confidence}% ({pair} {direction})")
        return

    signal["confidence_pct"] = confidence
    log_signal_decision(pair, direction, abs(signal["score"]), confidence, True, "Fired")
...
async def start_api_server():
    ...
    try:
        ...
        print(f"⚠️ Port {port} in use — API server skipped (another instance running)")
    ...
    await site.start()
    print("🌐 Stats API running on port 8080")
