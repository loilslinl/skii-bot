...
min_score        = 6             # minimum score to fire signal (changeable)
pair_cooldown    = 5 * 60        # cooldown per pair in seconds (changeable)
...
# Per-indicator cooldown: {indicator_pair_key: datetime}
indicator_cooldowns: dict = {}

# Global last signal time — prevent spamming
last_signal_time: datetime = None
GLOBAL_SIGNAL_COOLDOWN = 30  # seconds between any two signals

def indicator_on_cooldown(indicator: str, pair: str) -> bool:
    key  = f"{indicator}:{pair}"
    last = indicator_cooldowns.get(key)
    if last is None:
        return False
    return (datetime.now(timezone.utc) - last).total_seconds() < pair_cooldown

def set_indicator_cooldown(indicator: str, pair: str):
    indicator_cooldowns[f"{indicator}:{pair}"] = datetime.now(timezone.utc)
...
# ── Message Builders ──────────────────────────────────────────────────────────
# ── Independent Indicator Checks ─────────────────────────────────────────────
def check_rsi(data: dict) -> tuple:
...
def check_ema(data: dict) -> tuple:
    if data["ema9"] > data["ema21"]:
        return "CALL", "EMA9 crossed above EMA21 — Bullish"
    elif data["ema9"] < data["ema21"]:
        return "PUT", "EMA9 crossed below EMA21 — Bearish"
    return None, None

def check_macd(data: dict) -> tuple:
    if data["macd_hist"] > 0:
        return "CALL", f"MACD Histogram Positive — Bullish Momentum"
    elif data["macd_hist"] < 0:
        return "PUT", f"MACD Histogram Negative — Bearish Momentum"
    return None, None

def check_bollinger(data: dict) -> tuple:
    price, bb_upper, bb_lower = data["price"], data["bb_upper"], data["bb_lower"]
    if price < bb_lower:
        return "CALL", "Price Below Lower Bollinger Band — Bounce Expected"
    elif price > bb_upper:
        return "PUT", "Price Above Upper Bollinger Band — Reversal Expected"
    return None, None

def check_stochastic(data: dict) -> tuple:
    k, d = data["stoch_k"], data["stoch_d"]
    if k < 20 and k > d:
        return "CALL", f"Stochastic {k:.1f} — Oversold Crossover"
    elif k > 80 and k < d:
        return "PUT", f"Stochastic {k:.1f} — Overbought Crossover"
    return None, None

def check_adx(data: dict) -> tuple:
    adx, di_plus, di_minus = data["adx"], data["di_plus"], data["di_minus"]
    if adx > 25:
        if di_plus > di_minus:
            return "CALL", f"ADX {adx:.1f} — Strong Uptrend Confirmed"
        else:
            return "PUT", f"ADX {adx:.1f} — Strong Downtrend Confirmed"
    return None, None

def check_cci(data: dict) -> tuple:
    cci = data["cci"]
    if cci < -100:
        return "CALL", f"CCI {cci:.0f} — Oversold Zone"
    elif cci > 100:
        return "PUT", f"CCI {cci:.0f} — Overbought Zone"
    return None, None

def check_williams(data: dict) -> tuple:
    wr = data["williams_r"]
    if wr < -80:
        return "CALL", f"Williams %R {wr:.1f} — Oversold"
    elif wr > -20:
        return "PUT", f"Williams %R {wr:.1f} — Overbought"
    return None, None
...
INDICATOR_CHECKS = {
    ...
    "EMA":        check_ema,
    "MACD":       check_macd,
    "Bollinger":  check_bollinger,
    "Stochastic": check_stochastic,
    "ADX":        check_adx,
    "CCI":        check_cci,
    "Williams":   check_williams,
}
...
def fetch_indicator_data(pair: str) -> dict:
    """Fetch and compute all indicator values for a pair."""
    ticker = OTC_PAIRS[pair]
    df = yf.download(ticker, period="5d", interval="5m", progress=False, auto_adjust=True)
    if df.empty or len(df) < 30:
        raise ValueError(f"Not enough data for {pair}")
    ...
    bb_u  = float((sma + 2*std).iloc[-1])
    bb_l  = float((sma - 2*std).iloc[-1])

    k_val = 100 * (close - low.rolling(14).min()) / (high.rolling(14).max() - low.rolling(14).min())
    stk   = float(k_val.iloc[-1])
    std_d = float(k_val.rolling(3).mean().iloc[-1])
    ...
    di_p    = 100 * dm_p.rolling(14).mean() / atr_val
    di_m    = 100 * dm_m.rolling(14).mean() / atr_val
    dx      = 100 * (di_p - di_m).abs() / (di_p + di_m)
    adx_val = float(dx.rolling(14).mean().iloc[-1])

    typical = (high + low + close) / 3
    mad     = typical.rolling(20).apply(lambda x: abs(x - x.mean()).mean())
    cci_val = float(((typical - typical.rolling(20).mean()) / (0.015 * mad)).iloc[-1])

    highest = high.rolling(14).max()
    lowest  = low.rolling(14).min()
    wr_val  = float((-100 * (highest - close) / (highest - lowest)).iloc[-1])

    delta   = close.diff()
    gain    = delta.clip(lower=0).rolling(14).mean()
    loss    = (-delta.clip(upper=0)).rolling(14).mean()
    rsi_val = round(float((100 - 100 / (1 + gain/loss)).iloc[-1]), 2)
    ...
    return {
        ...
        "stoch_k":    round(stk, 1),
        "stoch_d":    round(std_d, 1),
        "adx":        round(adx_val, 2),
        "di_plus":    float(di_p.iloc[-1]),
        "di_minus":   float(di_m.iloc[-1]),
        "cci":        round(cci_val, 1),
...
def build_signal_msg(pair: str, direction: str, indicator: str, reason: str, entry_price: float, time_str: str) -> str:
    dir_block  = (
        "╔══════════════════╗\n║   📈  C A L L    ║\n╚══════════════════╝" if direction == "CALL"
        else "╔══════════════════╗\n║   📉   P U T    ║\n╚══════════════════╝"
    )
    ind_emoji = INDICATOR_EMOJI.get(indicator, "📊")
...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    """Scans all pairs with all 8 indicators independently every 2 min."""
    if bot_paused:
        return
    ...
    for pair in active_pairs:
        ...
        try:
            data = await loop.run_in_executor(None, fetch_indicator_data, pair)
        except Exception as e:
            print(f"⚠️ Data fetch failed for {pair}: {e}")
            continue

        # Run each indicator independently
        for indicator_name, check_fn in INDICATOR_CHECKS.items():

            # Check per-indicator cooldown
            if indicator_on_cooldown(indicator_name, pair):
                continue

            direction, reason = check_fn(data)
            if direction is None:
                continue

            # Global cooldown — wait 30s between any two signals
            now = datetime.now(timezone.utc)
            if last_signal_time and (now - last_signal_time).total_seconds() < GLOBAL_SIGNAL_COOLDOWN:
                wait = GLOBAL_SIGNAL_COOLDOWN - (now - last_signal_time).total_seconds()
                await asyncio.sleep(wait)

            # Fire the signal
            set_indicator_cooldown(indicator_name, pair)
            last_signal_time = datetime.now(timezone.utc)
            time_str    = datetime.now(timezone.utc).strftime("%H:%M UTC")
            entry_price = data["price"]
            text        = build_signal_msg(pair, direction, indicator_name, reason, entry_price, time_str)

            try:
                sent_msg = await context.bot.send_message(chat_id=CHANNEL_ID, text=text, parse_mode="Markdown")

                # Track message ID for pinning
                global last_signal_message_id
                last_signal_message_id = sent_msg.message_id

                # Poll after signal
                await asyncio.sleep(1)  # small pause before poll
                ...
                print(f"✅ {indicator_name} signal fired: {pair} {direction}")
...
async def result_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=CHANNEL_ID,
        text=build_result_msg(pair, direction, entry_price, exit_price, pips_label, stats),
        parse_mode="Markdown"
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/signal — Force post signal\n"
        "/setpairs — Set active pairs\n"
        "/resetpairs — Restore all 9 pairs\n"
        "/setscore [n] — Set min confidence\n"
        "/cooldown [min] — Set pair cooldown\n"
        "/expiry [min] — Set expiry time\n\n"
        "📊 *Stats & Info*\n"
        "/status — Bot status\n"
        "/today — Today's trade history\n"
        "/winstreak — Current & best streak\n"
        "/drawdown — Win rate vs today\n"
        "/revenue [n] — Revenue tracker\n"
        "/stats — Post stats to group\n"
        "/weekly — Post weekly recap\n\n"
        "📣 *Communication*\n"
        "/broadcast [msg] — Announcement\n"
        "/warn [msg] — Market warning\n"
        "/maintenance — Maintenance mode\n"
        "/tip [msg] — Post custom tip\n\n"
        "🎮 *Fun & Engagement*\n"
        "/pin — Pin last signal in group 📌\n"
        "/motivate — Post motivational message 💪\n\n"
        "🛠 *Other*\n"
...
async def cmd_warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚠️ Warning posted to group.")
...
async def cmd_maintenance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n\n"
            f"🔧 *MAINTENANCE MODE*\n\n"
            f"Signals are temporarily offline for maintenance.\n"
            f"We'll be back shortly — thanks for your patience! 🙏\n\n"
            f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
            f"_🎿 Skii Pro Signals — Premium OTC Alerts_"
        )
    )
    await update.message.reply_text("🔧 Maintenance mode on — signals paused. Use /resume when ready.")
...
async def cmd_manualtip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📚 Tip posted to group.")
...
async def cmd_drawdown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d_total == 0:
        await update.message.reply_text("📊 No trades today yet.", parse_mode="Markdown")
        return
    if diff > 15:
        status = f"⚠️ *Significant drawdown today!* Down {diff:.1f}% from average."
    elif diff > 5:
        status = f"📉 *Slight drawdown.* Down {diff:.1f}% from average."
    else:
        status = f"✅ *Performing normally.* Within {abs(diff):.1f}% of average."
    await update.message.reply_text(
        f"📊 *Drawdown Report*\n\n"
        f"All time win rate : *{all_wr:.1f}%*\n"
        f"Today's win rate  : *{day_wr:.1f}%*\n"
...
async def cmd_forceresult(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if outcome not in ["WIN", "LOSS"]:
        await update.message.reply_text("⚠️ Must be WIN or LOSS.", parse_mode="Markdown")
        return
    is_win = outcome == "WIN"
    stats  = load_stats()
    stats  = maybe_reset_daily(stats)
    ...
    await update.message.reply_text(f"{emoji} Manual {outcome} recorded and posted to group.")


async def cmd_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    if not last_signal_message_id:
        await update.message.reply_text("⚠️ No signal has been posted yet to pin.", parse_mode="Markdown")
        return
    try:
        await context.bot.pin_chat_message(
            chat_id=CHANNEL_ID,
            message_id=last_signal_message_id,
            disable_notification=False,
        )
        await update.message.reply_text("📌 Last signal pinned to the group!")
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to pin: `{e}`\n\nMake sure the bot is admin with pin permission.", parse_mode="Markdown")
...
async def cmd_motivate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"_{quote}_\n\n"
            f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
            f"💪 *Keep grinding. Skii Pro is with you every step.*\n\n"
            f"_🎿 Skii Pro Signals — Premium OTC Alerts_"
        )
    )
    await update.message.reply_text("💪 Motivation posted to group!")
