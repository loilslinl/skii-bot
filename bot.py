...
time_filter_enabled   = True    # only fire during best hours
...
def get_best_hours() -> set:
    """Returns hours with win rate above 55% and at least 5 trades."""
    stats = load_session_stats()
    good  = set()
...
# ── M5 Trend Filter ───────────────────────────────────────────────────────────
def get_m5_trend(ticker: str) -> str:
    """Returns 'UP', 'DOWN' or 'NEUTRAL' based on M5 EMA trend."""
    try:
        ...
        if ema8 > ema21 * 1.0002:
            return "UP"
        elif ema8 < ema21 * 0.9998:
            return "DOWN"
        return "NEUTRAL"
    except Exception:
        return "NEUTRAL"
...
def detect_candle_pattern(ticker: str) -> str:
    """Detects basic reversal patterns on M1. Returns 'CALL', 'PUT' or 'NONE'."""
    try:
...
def check_drawdown_protection(stats: dict) -> bool:
    """Returns True if bot should pause due to daily losses."""
    daily_losses = stats.get("daily_losses", 0)
    if daily_losses >= daily_loss_limit:
        print(f"🛑 Drawdown protection — {daily_losses} losses today, pausing")
        return True
    return False
...
def build_signal_msg(pair: str, signal: dict, time_str: str) -> str:
    ...
    m5_emoji    = "📈" if m5_trend == "UP" else "📉" if m5_trend == "DOWN" else "➡️"
    ...
    return (
        ...
        f"🎯 *Accuracy*  »  `{accuracy}`\n"
        f"🧠 *Sim Conf*  »  `{conf_str}`\n"
        f"{m5_emoji} *M5 Trend*  »  `{m5_trend}`\n\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
        f"📊 *ANALYSIS* ({votes}/3 indicators)\n\n"
        f"{reasons_txt}\n\n"
        f"📉 *Key Values*\n"
        f"   • RSI (7)      : `{signal.get('rsi', 'N/A')}`\n"
        f"   • Stoch (5,3,3): `{signal.get('stoch_k', 'N/A')}`\n"
        f"   • BB Lower     : `{signal.get('bb_lower', 'N/A')}`\n\n"
        f"{'🔥🔥🔥' if confidence and confidence >= 80 else '🔥🔥' if confidence and confidence >= 70 else '🔥'} *{'HIGH' if confidence and confidence >= 80 else 'GOOD' if confidence and confidence >= 70 else 'MODERATE'} CONFIDENCE* — {votes}/3  |  {conf_str} sim\n"
        f"{martingale}\n\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    """Scans all pairs every 60s with M5 trend, time, candle and drawdown filters."""
    if bot_paused:
        return
    ...
    for pair in active_pairs:
        ...
        if trend_filter_enabled:
            ticker    = OTC_PAIRS[pair]
            m5_trend  = await loop.run_in_executor(None, get_m5_trend, ticker)
            direction = signal["direction"]
            if m5_trend != "NEUTRAL":
                if (direction == "CALL" and m5_trend == "DOWN") or \
                   (direction == "PUT"  and m5_trend == "UP"):
                    log_signal_decision(pair, direction, abs(signal["score"]), 0, False, f"M5 trend against ({m5_trend})", signal.get("reasons", []))
                    print(f"⛔ {pair} {direction} blocked — M5 trend is {m5_trend}")
                    continue
            signal["m5_trend"] = m5_trend

        # ── Candlestick pattern filter ────────────────────────────────────────
        if candle_filter_enabled:
            ticker  = OTC_PAIRS[pair]
            pattern = await loop.run_in_executor(None, detect_candle_pattern, ticker)
            if pattern != "NONE" and pattern != signal["direction"]:
                log_signal_decision(pair, signal["direction"], abs(signal["score"]), 0, False, f"Candle pattern against ({pattern})", signal.get("reasons", []))
                print(f"⛔ {pair} blocked — candle pattern says {pattern}")
                continue
            if pattern != "NONE":
                signal["reasons"].append(f"Candle Pattern: {pattern} confirmation")
                signal["score"] += 1 if pattern == "CALL" else -1
    ...
    if confidence < 65:
        ...
        print(f"🧠 Signal skipped — MC confidence too low: {confidence}% ({pair} {direction})")
        return
    ...
    print(f"✅ Signal fired: {pair} {direction} | Score: {signal['score']} | MC: {confidence}% | M5: {signal.get('m5_trend','—')}")
...
async def cmd_filters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    log_admin("filters")
    global trend_filter_enabled, time_filter_enabled, candle_filter_enabled
    if context.args:
    ...
    await update.message.reply_text(
        f"🔧 *SMART FILTERS*\n\n"
        f"   {'✅' if trend_filter_enabled  else '❌'} Trend filter (M5)\n"
        f"   {'✅' if time_filter_enabled   else '❌'} Time filter\n"
        f"   {'✅' if candle_filter_enabled else '❌'} Candle pattern filter\n\n"
        f"_Toggle: `/filters trend` `/filters time` `/filters candle`_",
        parse_mode="Markdown"
    )
...
async def handle_stats(request):
    """Returns live bot stats as JSON for the Monte Carlo simulator."""
    stats = load_stats()
    total = stats["wins"] + stats["losses"]
    wr    = winrate(stats["wins"], stats["losses"])
    data  = {
...
async def handle_feed(request):
    """Returns live signal decision log."""
    return web.Response(
        text=json.dumps(list(reversed(signal_log))),
        content_type="application/json",
        headers={"Access-Control-Allow-Origin": "*"}
    )
...
async def start_api_server():
    ...
    try:
        ...
        await site.start()
        print(f"🌐 Stats API running on port {port}")
    except OSError:
