...
def get_mtf_trend(ticker: str) -> dict:
    """
    Checks M1, M5 and M15 trend direction using EMA.
    Returns dict with each timeframe's direction and overall agreement.
    """
    results = {}
    timeframes = {"M1": "1m", "M5": "5m", "M15": "15m"}
    periods    = {"M1": ("1d", 8, 21), "M5": ("5d", 8, 21), "M15": ("30d", 8, 21)}

    for tf, interval in timeframes.items():
        try:
            hist_period, fast, slow = periods[tf]
            df    = yf.download(ticker, period=hist_period, interval=interval, progress=False, auto_adjust=True)
    ...
    if up_count >= 2:
        results["overall"] = "UP"
        results["strength"] = up_count
    elif down_count >= 2:
        results["overall"] = "DOWN"
        results["strength"] = down_count
    else:
...
def check_volatility(ticker: str) -> dict:
    ...
    try:
        ...
        if atr_pct < 0.01:
            level, tradeable = "TOO_LOW", False    # dead market
        elif atr_pct < 0.05:
            ...
            level, tradeable = "TOO_HIGH", False   # too chaotic
...
def get_support_resistance(ticker: str) -> dict:
    """
    Finds key S&R levels using recent swing highs/lows.
    Returns nearest support, resistance, and whether price is near either.
    """
    try:
        df = yf.download(ticker, period="1d", interval="5m", progress=False, auto_adjust=True)
        if df.empty or len(df) < 20:
            return {"near_support": False, "near_resistance": False, "signal": "NONE"}
        ...
        price    = float(close.iloc[-1])
        # Find swing highs and lows over last 50 candles
        ...
        threshold  = (resistance - support) * 0.08  # within 8% of level

        near_support    = abs(price - support)    < threshold
        near_resistance = abs(price - resistance) < threshold

        signal = "NONE"
        if near_support:
            signal = "CALL"   # bounce up from support
        elif near_resistance:
            signal = "PUT"    # bounce down from resistance
...
def build_signal_msg(pair: str, signal: dict, time_str: str) -> str:
    ...
    mtf_str     = f"{mtf_overall} ({mtf.get('strength',0)}/3 TFs)"
    ...
    sr          = signal.get("sr", {})
    sr_str      = "Near Support 🟢" if sr.get("near_support") else "Near Resistance 🔴" if sr.get("near_resistance") else "No Key Level"
...
async def scanner_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── News filter ───────────────────────────────────────────────────────────
    if time_filter_enabled and is_news_time():
    ...
    for pair in active_pairs:
        ...
        if mtf["overall"] != "NEUTRAL":
            if (direction == "CALL" and mtf["overall"] == "DOWN") or \
               (direction == "PUT"  and mtf["overall"] == "UP"):
                ...
                continue
            # MTF agrees — boost score
            if (direction == "CALL" and mtf["overall"] == "UP") or \
               (direction == "PUT"  and mtf["overall"] == "DOWN"):
                signal["score"] += 1
                signal["reasons"].append(f"MTF Confirmed {mtf['overall']} ({mtf['strength']}/3 TFs)")
        ...
        if sr["signal"] != "NONE":
            if sr["signal"] == direction:
                signal["score"] += 1
                label = "Near Support" if sr["near_support"] else "Near Resistance"
                signal["reasons"].append(f"S&R: {label} ✅")
            elif sr["signal"] != direction:
                # Against S&R — slight penalty
                signal["score"] -= 1
